        ...
        <div>
          {user ? (
            <div className="flex items-center gap-4">
              ...
              {user.avatar_url && (
                <img src={user.avatar_url} alt="Avatar" className="w-8 h-8 rounded-full shadow-sm border border-slate-200" />
              )}
              <button onClick={logout} className="bg-slate-100 hover:bg-slate-200 text-slate-700 px-4 py-2 rounded-full font-medium transition-colors text-sm shadow-sm cursor-pointer">
                Logout
              </button>
            </div>
          ) : (
